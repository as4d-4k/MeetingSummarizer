...
import apiClient, { saveLanguagePreferences } from '../api/client';
...
export default function Register() {
  ...
  const [hintsStatus, setHintsStatus] = useState('');
  // hintsStatus: '' | 'generating' | 'done' | 'error'
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 2. Save all extra info to localStorage (for Admin page display)
      const extraInfo = {
      ...
      // 3. Save language preferences to backend + trigger hint generation
      setHintsStatus('generating');
      try {
        await saveLanguagePreferences({
          primary_language:   LANGUAGE_CODES[form.primaryLanguage]   || 'en-US',
          secondary_language: LANGUAGE_CODES[form.secondaryLanguage] || 'en-US',
          industry:           INDUSTRY_MAP[form.industry]             || 'general',
        });
        // Backend will trigger Celery task to generate hints async
        setHintsStatus('done');
      } catch (hintErr) {
        // Don't block signup if hint generation fails
        console.warn('Hint generation request failed:', hintErr);
        setHintsStatus('error');
      }
  ...
  return (
    <div className="auth-root">
        ...
        <div className="auth-card">
          <div className="auth-card-header" style={{ position: 'relative' }}>
            ...
            <p className="auth-subtitle">
              ...
              {step === 2 && 'Step 2 of 3 — About your company'}
              {step === 3 && 'Step 3 of 3 — Language & meeting setup'}
            </p>
              ...
              <div className="step-content">
                <div className="auth-field">
                  ...
                  <input type="text" value={form.fullName} onChange={update('fullName')}
                    placeholder="Muhammad Umer" className="auth-input" required />
                </div>
                <div className="auth-field">
              ...
              <div className="step-content">
                <div className="auth-field">
                  ...
                  <input type="text" value={form.companyName} onChange={update('companyName')}
                    placeholder="TechCorp Pvt Ltd" className="auth-input" required />
                </div>
                <div className="auth-field">
            ...
            {step === 3 && (
              <div className="step-content">
                ...
                <div style={{
                  ...
                  🤖 On signup, AI will generate speech recognition hints for your
                  selected language and industry. This improves transcription accuracy
                  from the very first word in your meetings.
                </div>
                ...
                <div className="auth-field">
                  <label className="auth-label">Secondary Language (for code-switching)</label>
                  <div className="auth-select-wrapper">
                ...
                {/* Hint generation status */}
                {hintsStatus === 'generating' && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: '#9B8FFF' }}>
                    <span className="auth-spinner" style={{ borderColor: 'rgba(108,99,255,0.3)', borderTopColor: '#6C63FF' }} />
                    Generating AI hints for your language…
                  </div>
                )}
                {hintsStatus === 'done' && (
                  <div style={{ fontSize: 12, color: '#00C896' }}>
                    ✅ AI hints generated successfully
                  </div>
                )}
              </div>
            )}
      ...
      <style>{`
        .auth-root { min-height:100vh; display:flex; align-items:center; justify-content:center; background:var(--bg-void); position:relative; overflow:hidden; padding:24px; font-family:var(--font-body); }
        ...
        .noise-overlay { position:fixed; inset:0; pointer-events:none; z-index:0; opacity:0.5; background-image:url("data:image/svg+xml,%3Csvg viewBox='0 0 256 256' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='noise'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.9' numOctaves='4' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23noise)' opacity='0.035'/%3E%3C/svg%3E"); }
        .auth-container { position:relative; z-index:1; width:100%; display:flex; flex-direction:column; align-items:center; gap:28px; animation:fadeSlideUp 0.5s ease both; }
        ...
        .auth-logo-mark { width:48px; height:48px; border-radius:14px; flex-shrink:0; background:linear-gradient(135deg,#6C63FF,#9B8FFF); display:flex; align-items:center; justify-content:center; color:#fff; box-shadow:0 0 0 1px rgba(108,99,255,0.4),0 8px 28px rgba(108,99,255,0.3); }
        ...
        .auth-card { width:100%; background:var(--bg-card); border:1px solid var(--border-subtle); border-radius:20px; padding:32px; box-shadow:0 24px 64px rgba(0,0,0,0.4); }
        ...
        .step-indicators { position:absolute; right:0; top:0; display:flex; gap:6px; }
        .step-dot { width:8px; height:8px; border-radius:50%; background:var(--bg-surface); border:1px solid var(--border-subtle); transition:all 0.3s; }
        .step-dot.active { background:#6C63FF; border-color:#6C63FF; box-shadow:0 0 8px rgba(108,99,255,0.5); }
        .auth-form { display:flex; flex-direction:column; gap:18px; }
        .step-content { display:flex; flex-direction:column; gap:18px; animation:fade 0.3s ease; }
        @keyframes fade { from{opacity:0} to{opacity:1} }
        .auth-field { display:flex; flex-direction:column; gap:7px; }
        .auth-label { font-size:12px; font-weight:500; color:var(--text-secondary); letter-spacing:0.02em; }
        .auth-input { background:var(--bg-deep); border:1px solid var(--border-subtle); border-radius:var(--radius-md); padding:11px 14px; color:var(--text-primary); font-size:14px; font-family:var(--font-body); width:100%; outline:none; transition:all 0.2s; }
        ...
        .auth-select-wrapper svg { position:absolute; right:14px; top:50%; transform:translateY(-50%); pointer-events:none; color:var(--text-muted); }
        .auth-error { display:flex; align-items:center; gap:8px; font-size:13px; color:#FF6B6B; background:rgba(255,107,107,0.08); border:1px solid rgba(255,107,107,0.2); border-radius:var(--radius-md); padding:10px 14px; }
        .auth-actions { display:flex; gap:12px; margin-top:4px; }
        .auth-btn-secondary { padding:12px 20px; background:var(--bg-surface); border:1px solid var(--border-subtle); border-radius:var(--radius-md); color:var(--text-primary); font-family:var(--font-body); font-size:14px; font-weight:500; cursor:pointer; transition:all 0.2s; }
        .auth-submit { display:flex; align-items:center; justify-content:center; gap:8px; padding:12px; background:linear-gradient(135deg,#6C63FF,#9B8FFF); border:none; border-radius:var(--radius-md); color:#fff; font-family:var(--font-body); font-size:14px; font-weight:500; cursor:pointer; box-shadow:0 4px 20px rgba(108,99,255,0.35); transition:all 0.2s; }
        ...
        .auth-footer { display:flex; align-items:center; justify-content:center; gap:6px; margin-top:24px; font-size:13px; color:var(--text-muted); padding-top:20px; border-top:1px solid var(--border-subtle); }
        .auth-link { color:#9B8FFF; font-weight:500; text-decoration:none; transition:color 0.2s; }
...
function SignInIcon() { return <svg width="15" height="15" viewBox="0 0 16 16" fill="none"><path d="M8 8a3 3 0 1 0 0-6 3 3 0 0 0 0 6Zm2-3a2 2 0 1 1-4 0 2 2 0 0 1 4 0Zm4 8c0 1-1 1-1 1H3s-1 0-1-1 1-4 6-4 6 3 6 4Z" fill="currentColor"/></svg>; }
